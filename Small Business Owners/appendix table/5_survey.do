...
********
u "$proc/combined_survey.dta", clear
...
esttab A B C D using "/Users/gabrielbo/Dropbox/src 2024/Small Business Owners/5_single_employee_reg.tex", replace ///
...
eststo clear
eststo A: reg pid start tookover nonlaborer gigworker [pweight = weight]

eststo B: reghdfe pid start tookover nonlaborer gigworker [pweight = weight], absorb(i.age_group i.race_cat i.hispanic_latino i.gender2 i.total_income i.education_level2 i.region) 
 estadd local control = "X"
 
eststo C: reg biden_or start tookover nonlaborer gigworker [pweight = weight]

eststo D: reghdfe biden_or start tookover nonlaborer gigworker [pweight = weight], absorb(i.age_group i.race_cat i.hispanic_latino i.gender2 i.total_income i.education_level2 i.region) 
 estadd local control = "X"
...
eststo a: reg pid current previous nonlaborer gigworker [pweight = weight]

eststo b: reghdfe pid current previous nonlaborer gigworker [pweight = weight], absorb(i.age_group i.race_cat i.hispanic_latino i.gender2 i.total_income i.education_level2 i.region) 
 estadd local control = "X"
 
eststo c: reg biden_or current previous nonlaborer gigworker [pweight = weight]

eststo d: reghdfe biden_or current previous nonlaborer gigworker [pweight = weight], absorb(i.age_group i.race_cat i.hispanic_latino i.gender2 i.total_income i.education_level2 i.region) 
 estadd local control = "X"
...
esttab A B a b C D c d using "/Users/gabrielbo/Dropbox/src 2024/Small Business Owners/5_current_takeover.tex", replace ///
		cells(b(fmt(%-10.3fc) star) se(par fmt(%-10.3fc))) ///
		stats(control N r2, fmt( 0 0 2)	///
		labels("Controls" "Observations" "R-squared"))	///
		mgroups("PID (6-point)" "Trump or Biden (3-point)",	///		
...
esttab A B a b C D c d using "/Users/gabrielbo/Dropbox/src 2024/Small Business Owners/5_current_takeover_employer.tex", replace ///
		cells(b(fmt(%-10.3fc) star) se(par fmt(%-10.3fc))) ///
		stats(control N r2, fmt( 0 0 2)	///
		labels("Controls" "Observations" "R-squared"))	///
		mgroups("PID (6-point)" "Trump or Biden (3-point)",	///		
